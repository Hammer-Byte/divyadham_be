...
class EventsController extends Controller
{
  ...
  public function eventList() {
    ...
    return DataTables::of($query)
        ->editColumn('event_details', function ($q) {
            ...
            } else if ($q->banner_upload_type == 'url') {
            ...
            } else if ($q->event_image_upload_type == 'url') {
  ...
  public function save(Request $request){
    $rules = [
        ...
        'banner_type_file' => 'required_if:banner_upload_type,file|mimes:jpg,jpeg,png',
        'banner_type_url' => 'nullable|required_if:banner_upload_type,url|url',
        'event_image_upload_type' => 'required|string',
        'event_image_type_file' => 'required_if:event_image_upload_type,file|mimes:jpg,jpeg,png',
        'event_image_type_url' => 'nullable|required_if:event_image_upload_type,url|url',
        'organizers' => 'required',
    ...
    $messages = [
        ...
        'banner_type_file.required_if' => 'The banner file field is required when upload type is file.',
        'banner_type_file.mimes' => 'Only jpg, jpeg, and png formats are allowed.',
        'banner_type_url.required_if' => 'The banner URL field is required when upload type is URL.',
        'banner_type_url.url' => 'Please enter a valid URL (e.g., https://example.com).',
        'event_image_upload_type.required' => 'The event image upload type field is required.',
        'event_image_type_file.required_if' => 'The event image file field is required when upload type is file.',
        'event_image_type_file.mimes' => 'Only jpg, jpeg, and png formats are allowed.',
        'event_image_type_url.required_if' => 'The event image URL field is required when upload type is URL.',
        'event_image_type_url.url' => 'Please enter a valid URL (e.g., https://example.com).',
    ...
    if ($request->banner_upload_type == 'file_upload') {
        $data['banner_image_url'] = storeFile($request->file('banner_type_file'), 'events');
    ...
    if ($request->event_image_upload_type == 'file_upload') {
        $data['event_image_url'] = storeFile($request->file('event_image_type_file'), 'events');
    }else if ($request->event_image_upload_type == 'url') {
  ...
  public function eventEdit($id){
    ...
    $data['method'] = 'Edit';
    $data['event'] = Events::selectRaw("*, DATE_FORMAT(start_date, '%Y-%m-%d') as start_date, DATE_FORMAT(end_date, '%Y-%m-%d') as end_date")->where('id', $id)->first();
    $data['organizers'] = Organizer::where('status',1)->get();
