...
<div class="col-12">
    <div class="d-flex justify-content-between">
        ...
        <div class="">
            <a href="{{route('manage.donationMediaCreate',['donationId' => $donation->id])}}" title="Add"><i class="text-primary bx bx-plus bx-md"></i></a>
        </div>
    </div>
    <div class="row">
        <div class="table-responsive text-nowrap">
            <table class="table" id="donationMediaTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Media Type</th>
...
@section('extraScript')
<script>
    function donationMediaTable() {
        if (!$.fn.DataTable.isDataTable("#donationMediaTable")) {
            $('#donationMediaTable').DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('manage.donationMediaList',['donationId' => $donation->id]) }}",
                "columns": [
                    ...
                    { "data": null, "className": "text-center", "orderable": false, "searchable": false,
                        "render": function (data, type, row) {
                                    var tableName = "donation_media"
                                    var editUrl = "{{ route('manage.donationMediaEdit',['donationId' => $donation->id, 'id' => ':id']) }}".replace(':id', row.id);
