...
@section('content')
    ...
    <div class="card">
        ...
        <form id="{{@$method == 'Add' ? 'event-media-form-add' : 'event-media-form-edit'}}" method="post" action="{{@$method == 'Add' ? route('manage.eventMediaAdd') : route('manage.eventMediaUpdate')}}" enctype="multipart/form-data">
            @csrf
            @if(@$method == 'Edit')
                <input type="hidden" value="{{$eventMedia->id}}" name="id"/>
            @endif
                    ...
                    <div class="col-6">
                        <div class="col-12 mb-1">
                            ...
                            <select id="media_upload_type" name="media_upload_type" class="form-control select2">
                                @php
                                    $selectedMediaUploadType = @$eventMedia ? @$eventMedia->media_upload_type : old('media_upload_type');
                                @endphp
                        ...
                        <div class="col-12 mb-1 media_type_url_wrapper d-none">
                            ...
                            <input type="text" class="form-control" id="media_type_url" name="media_type_url" value="{{@$eventMedia ? @$eventMedia->media_url : old('media_type_url')}}" aria-describedby="defaultFormControlHelp">
                        </div>
                        <div class="col-12 mb-1">
                            <label for="media_type" class="form-label">Media Type</label>
                            <select id="media_type" name="media_type" class="form-control select2">
                                @php
                                    $selectedMediaType = @$eventMedia ? @$eventMedia->media_type : old('media_type');
                                @endphp
                    ...
                    <div class="col-6">
                        <div class="col-12 mb-1">
                            ...
                            <select id="position" name="position" class="form-control select2">
                                @php
                                    $selectedPosition = @$eventMedia ? @$eventMedia->position : old('position');
                                @endphp
                                <option value="">Select Media Type</option>
                                @php
                                    if (isset($eventMedia)) {
                        ...
                        <div class="col-12 mb-1">
                            ...
                            <select id="status" name="status" class="form-control select2">
                                @php
                                    $selectedStatus = @$eventMedia ? @$eventMedia->status : old('status');
                                @endphp
            ...
            <div class="card-footer text-end">
                <a type="button" class="btn btn-secondary m-1" href="{{route('manage.eventView',['id' => $eventId, 'currentTab' => 'navs-pills-event-media'])}}">Cancel</a>
...
<script>
    $(document).ready(function() {
        ...
        $("#event-media-form-edit").validate({
            rules: {
                media_upload_type: {
                    required: true,
                },
