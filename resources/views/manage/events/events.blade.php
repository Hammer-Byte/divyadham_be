...
@section('content')
    ...
    <div class="card">
        <div class="d-flex justify-content-between">
            ...
            <div class="m-3">
                <a href="{{route('manage.eventCreate')}}" title="Add"><i class="text-primary bx bx-plus bx-md"></i></a>
            </div>
        </div>
        <div class="row m-2">
    ...
    $(document).ready(function() {
        $('#eventTable').DataTable({
            ...
            "columns": [
                ...
                { "data": "event_details", "className": "", "searchable": true, "orderable": true,
                    "render": function (data, type, row) {
                        return `<div>
                                    <span>Start Date: ${data.state}</span><br/>
                                    <span>End Date: ${data.district}</span><br/>
                                    <span>Location: ${data.location}</span><br/>
                                    <span>Lat-Long: ${data.lat}, ${data.long}</span><br/>
                                    <span>Banner Image: <a href="${data.banner_image_url}" target="_blank">Click Here!</a></span><br/>
                                    <span>Event Image: <a href="${data.event_image_url}" target="_blank">Click Here!</a></span>
                                </div>`
                }
                },
                ...
                { "data": null, "className": "text-center", "orderable": false, "searchable": false,
                    "render": function (data, type, row) {
                                var tableName = "{{$table_name}}"
                                var editUrl = "{{ route('manage.eventEdit', ':id') }}".replace(':id', row.id);
                                var viewUrl = "{{ route('manage.eventView', ':id') }}".replace(':id', row.id);
                                return `
