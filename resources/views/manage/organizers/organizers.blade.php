...
@section('content')
    ...
    <div class="card">
        <div class="d-flex justify-content-between">
            ...
            <div class="m-3">
                <a href="{{route('manage.organizerCreate')}}" title="Add"><i class="text-primary bx bx-plus bx-md"></i></a>
            </div>
        </div>
        <div class="row m-2">
            <div class="table-responsive text-nowrap">
                <table class="table" id="organizersTable">
                    <thead>
                        <tr>
                            <th>#</th>
...
<script>
    $(document).ready(function() {
        $('#organizersTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('manage.organizerList') }}",
            "columns": [
                ...
                { "data": null, "className": "text-center", "orderable": false, "searchable": false,
                    "render": function (data, type, row) {
                                var tableName = "{{$table_name}}"
                                var editUrl = "{{ route('manage.organizerEdit', ':id') }}".replace(':id', row.id);
                                return `
