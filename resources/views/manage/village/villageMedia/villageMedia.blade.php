...
<div class="col-12">
    <div class="d-flex justify-content-between">
        ...
        <div class="">
            <a href="{{route('manage.villageMediaCreate',['villageId' => $village->id])}}" title="Add"><i class="text-primary bx bx-plus bx-md"></i></a>
        </div>
    </div>
    <div class="row">
        <div class="table-responsive text-nowrap">
            <table class="table" id="villageMediaTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Media Type</th>
...
@section('extraScript')
<script>
    function villageMediaTable() {
        if (!$.fn.DataTable.isDataTable("#villageMediaTable")) {
            $('#villageMediaTable').DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('manage.villageMediaList',['villageId' => $village->id]) }}",
                "columns": [
                    ...
                    { "data": null, "className": "text-center", "orderable": false, "searchable": false,
                        "render": function (data, type, row) {
                                    var tableName = "village_media"
                                    var editUrl = "{{ route('manage.villageMediaEdit',['villageId' => $village->id, 'id' => ':id']) }}".replace(':id', row.id);
