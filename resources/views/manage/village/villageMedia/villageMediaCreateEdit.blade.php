...
@section('content')
    ...
    <div class="card">
        ...
        <form id="{{@$method == 'Add' ? 'village-media-form-add' : 'village-media-form-edit'}}" method="post" action="{{@$method == 'Add' ? route('manage.villageMediaAdd') : route('manage.villageMediaUpdate')}}" enctype="multipart/form-data">
            @csrf
            @if(@$method == 'Edit')
                <input type="hidden" value="{{$villageMedia->id}}" name="id"/>
            @endif
                    ...
                    <div class="col-6">
                        <div class="col-12 mb-1">
                            ...
                            <select id="media_upload_type" name="media_upload_type" class="form-control select2">
                                @php
                                    $selectedMediaUploadType = @$villageMedia ? @$villageMedia->media_upload_type : old('media_upload_type');
                                @endphp
                        ...
                        <div class="col-12 mb-1 media_type_url_wrapper d-none">
                            ...
                            <input type="text" class="form-control" id="media_type_url" name="media_type_url" value="{{@$villageMedia ? @$villageMedia->media_url : old('media_type_url')}}" aria-describedby="defaultFormControlHelp">
                        </div>
                        <div class="col-12 mb-1">
                            <label for="media_type" class="form-label">Media Type</label>
                            <select id="media_type" name="media_type" class="form-control select2">
                                @php
                                    $selectedMediaType = @$villageMedia ? @$villageMedia->media_type : old('media_type');
                                @endphp
                    ...
                    <div class="col-6">
                        <div class="col-12 mb-1">
                            ...
                            <select id="position" name="position" class="form-control select2">
                                @php
                                    $selectedPosition = @$villageMedia ? @$villageMedia->position : old('position');
                                @endphp
                                <option value="">Select Media Type</option>
                                @php
                                    if (isset($villageMedia)) {
                                        $addCount = 1;
                                    }else{
                                        $addCount = 2;
                                    }
                        ...
                        <div class="col-12 mb-1">
                            ...
                            <select id="status" name="status" class="form-control select2">
                                @php
                                    $selectedStatus = @$villageMedia ? @$villageMedia->status : old('status');
                                @endphp
            ...
            <div class="card-footer text-end">
                <a type="button" class="btn btn-secondary m-1" href="{{route('manage.villageView',['id' => $villageId, 'currentTab' => 'navs-pills-village-media'])}}">Cancel</a>
...
<script>
    $(document).ready(function() {
        ...
        $("#village-media-form-add").validate({
            rules: {
                media_upload_type: {
                    required: true,
                },
        ...
        $("#village-media-form-edit").validate({
            rules: {
                media_upload_type: {
                    required: true,
                },
